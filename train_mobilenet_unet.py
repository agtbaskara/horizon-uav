...
def create_model():
    ...
    encoder = tf.keras.applications.MobileNetV2(input_tensor=inputs, include_top=False, weights="imagenet")
    encoder_output = encoder.get_layer("out_relu").output

    x = encoder_output

    # Skip Connection
    skip_connection_names = ['block_12_add', 'block_5_add', 'block_2_add', 'expanded_conv_project_BN', 'input_image']
    filter_number = [64, 32, 16, 8, 4]
    ...
    for i in range(0, 5):
        ...
        x = tf.keras.layers.Conv2D(filter_number[i], (3, 3), padding='same')(x)
